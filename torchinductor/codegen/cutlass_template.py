...
class CutlassTemplateKernel(TritonKernel):
    def __init__(self, node: ir.ExternKernel, *groups):
        super(CutlassTemplateKernel, self).__init__(*groups)
        self.node = node
    ...
    def codegen_body(
        ...
        print(f"self.assign_block_numel().getvalue(): {self.assign_block_numel().getvalue()}")
        self.pointwise_code.splice(self.assign_block_numel())
        print(f"self.body.getvalue(): {self.body.getvalue()}")
...
def template_codegen(scheduler, scheduler_node):
    """
    codegen function for cutlass templates
    scheduler: Scheduler
    ...
    could_remove_kernel_buf = False
    fusable_nodes = []
    with scheduler.kernel(CutlassTemplateKernel(scheduler_node.node, *group)) as kernel:
        # map const args/ shape/ strides to kernel args
        kernel.map_args()
        # set self.args name to match the CutlassTemplateKernel's args names
        kernel.rename_vars()
        # update node dep from StarDep to MemoryDep
        scheduler_node.update_dep_type()
        # mark node of CutlassTemplateKernel as fusable and update fusable_deps
